...
2.Sorting and subsetting : [[]] , double bracket , 因為結果要是dataframe
```
sorting:
...
each label : row name 下面的名字
each variable : column's name下面的名字
df.groupby('column name') ... 變成一個panda series，再去做column操作
df.groupby(['cola','colb'])['colc'].sum()...多個variable的groupby..變成一個pandas dataframe,因為bracket select dataframe

ps.[numply float,numpy float, numpy float]也算是ndarray,不算list
ps..groupby() 後的type為pandas series 
ps..sum()後的type為numpy.float
ps.pandas seires and ndarray都可以做element的計算
